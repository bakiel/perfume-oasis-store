...
async function restoreAllProducts() {
  console.log('Restoring all 33 products...\n')
  ...
  // All 33 products with real images
  const products = [
  ...
  for (const product of products) {
    ...
    const { error } = await supabase.from('products').insert({
      sku: `PO-${timestamp}-${randomStr}`.toUpperCase(),
      name: product.name,
      slug: product.name.toLowerCase().replace(/ /g, '-') + '-' + timestamp,
      ...
      description: `Experience the luxury of ${product.name}`,
      short_description: `${product.brand} ${product.name}`
    })
  ...
  console.log(`\n✅ Restored ${successCount} of ${products.length} products!`)
}
